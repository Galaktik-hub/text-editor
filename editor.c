...
// Editor row, stores a line of text as a pointer
typedef struct erow {
    int idx;
    int size;
    int rsize;
...
void editorUpdateSyntax(erow *row) {
    ...
    while(i < row->rsize) {
        ...
        if (E.syntax->flags & HL_HIGHLIGHT_STRINGS) {
            if (in_string) {
                row->hl[i] = HL_STRING;
                // If the user escape a character, we skip it
                if (c == '\\' && i + 1 < row->rsize) {
...
// The callback function for the incremental search
...
void editorProcessKeypress() {
    ...
    switch (c) {
        ...
        case CTRL_KEY('q'):
            ...
            // We clear the window before exiting
            write(STDOUT_FILENO, "\x1b[2J", 4);
            write(STDOUT_FILENO, "\x1b[H", 3);
            exit(0);
            break;
